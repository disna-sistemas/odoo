...
class dsnStockProductionLot(models.Model):
    ...
    def _compute_semi_lots(self):
        for record in self:
            if record.id:
                ...
                record.dsn_semi_comp_unique_lot_ids = track_obj.search([('product_lot','=',record.id),('component.product_tmpl_id.dsncat2_id.name','=','SEMI')]).mapped('component_lot')
    ...
    dsn_semi_comp_unique_lot_ids = fields.One2many(comodel_name='stock.production.lot',
                                                  compute='_compute_semi_lots',
                                                   store=True)
